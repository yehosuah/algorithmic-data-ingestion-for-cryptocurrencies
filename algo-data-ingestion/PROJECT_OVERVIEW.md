...
- **Live readiness**: Deployment contracts, launch ladder CLIs, runtime risk/intent-ledger/deadlock modules, and the expanded audit/metrics footprint enforce kill-/safe-mode policies, signed audit provenance, reconciliation safe-mode latching, and coverage-focused deadlock mitigations before promoting BTC/ETH/SOL to live trading.
...
- **Trading controls**: `app/trading/service.py` now enforces kill-/safe-mode envs, runtime risk gating (turnover/exposure/cooldown/per-symbol limits), Redis intent ledger dedupe, reconciliation safe-mode latching, programmable deadlock policies, and HMAC-signed audit logging. Stage-0 overrides now reduce `cooldown_minutes_after_exit` to 1 and `cooldown_minutes_after_loss` to 5 and the dry-run compose mirrors the live ladder with BTC/ETH/SOL all marked as primary symbols (`TRADING_SHADOW_SYMBOLS=[]`). Reports from `analysis.*` CLIs plus Prometheus metrics (`trading_deadlock_*`, `trading_safe_mode_latched`, `trading_risk_blocked_total`, `trading_intent_ledger_state_total`, `trading_reconcile_runs_total`) mirror the deployment contract's live invariants.
