...
- **Equity-fraction sizing** – Stage-0 risk overrides now run `sizing_mode=equity_fraction` with `capital=100`, base notionals 20/15/12, `max_total_notional=80`, `equity_fraction=0.2`, `max_equity_fraction=0.3`, `compounding_step_usd=5`, per-symbol caps, and per-symbol trigger overrides (lower exit thresholds, longer holds) plus `cooldown_minutes_after_exit: 2` / `cooldown_minutes_after_loss: 5`.
- **Env/compose parity** – `configs/runtime_overrides/stage_0.yaml`, `docker-compose.yml`, and `configs/deployment_portfolio_contract.yaml` reflect the smaller notionals, longer holds (`max_hold_minutes: 90`, `take_profit_pct: 0.001`), and per-symbol overrides; `.dockerignore` now excludes reports/logs/experiments from build context to keep images lean.
...
- **Stage-0 guard tweak** – The runtime risk overrides now run equity-fraction sizing (`capital=100`, `equity_fraction=0.2`, `max_equity_fraction=0.3`, `compounding_step_usd=5`) with base notionals 20/15/12, `max_total_notional=80`, per-symbol trigger overrides, and `cooldown_minutes_after_exit: 2` / `cooldown_minutes_after_loss: 5` while the dry-run compose wires BTC/ETH/SOL as full primary entries so rehearsals track the intended live posture.
...
- **Shared trading decision logic** – Trading now routes through `app/trading/decision.py::decide_bar`, applying spread/hold/SL/TP checks consistently across live loops and offline sweeps; `TRADING_MODELS` accepts guard fields (max spread, stop loss, take profit, max hold, min hold overrides) and the default compose applies the same stage-0 values (base notionals 20/15/12 before equity scaling, `max_spread_bps=15/20/22`, `take_profit_pct=0.001`, `max_hold_minutes=90`, `shadow_mode=false`). Decision payloads now carry price/spread fields so exits respect execution context.
...
- `app/trading/service.py` consumes that queue, keeps per-symbol state (`app/trading/state.py`) with file/Redis/Postgres backends, logs audit events (`app/trading/audit.py`), and routes orders through the CCXT adapter with spread guardrails and dry-run toggles. The shared `app/trading/decision.py::decide_bar` applies entry/exit/hold/SL/TP/spread checks consistently across live loops and offline sweeps; `TRADING_MODELS` entries accept guard fields and the compose defaults mirror the live ladder (BTC/ETH/SOL primary entries, base notionals 20/15/12 before equity scaling, symbol-specific spread caps, `shadow_mode=false`).
...
- **Trading controls**: `app/trading/service.py` now enforces kill-/safe-mode envs, runtime risk gating (turnover/exposure/cooldown/per-symbol limits), Redis intent ledger dedupe, reconciliation safe-mode latching, programmable deadlock policies, richer audit payloads (expected/net PnL, quote source, `loss_guard` hits), and HMAC-signed logging. Stage-0 overrides now run equity-fraction sizing (capital 100, base notionals 20/15/12, `max_total_notional=80`, `equity_fraction=0.2`, `max_equity_fraction=0.3`, `compounding_step_usd=5`), per-symbol trigger overrides, and `cooldown_minutes_after_exit: 2` / `cooldown_minutes_after_loss: 5` while the dry-run compose mirrors the live ladder with BTC/ETH/SOL all marked as primary symbols (`TRADING_SHADOW_SYMBOLS=[]`). Reports from `analysis.*` CLIs (including `analysis.live_readiness_check`, `analysis.acceptance_trade_proof`, `analysis.exit_attribution_report`, `analysis.project_stance_snapshot`, plus the new forensics/alignment chain) and Prometheus metrics (`trading_deadlock_*`, `trading_safe_mode_latched`, `trading_risk_blocked_total`, `trading_intent_ledger_state_total`, `trading_reconcile_runs_total`) mirror the deployment contract's live invariants.
