...
> Update 2025-11-30 22:29 UTC: Stage-0 runtime risk overrides now set a 1-minute cooldown after exits and 5 minutes after losses, the dry-run compose promotes BTC/ETH/SOL to full primary entries (300 USDT notional, `max_spread_bps=10`, empty `TRADING_SHADOW_SYMBOLS`), and the bulky parquet datasets were dropped from git so they’re regenerated via the existing backfill/sanitize scripts.
...
- **Stage-0 guard tweak** – The runtime risk overrides now cut `cooldown_minutes_after_exit` to 1 and `cooldown_minutes_after_loss` to 5 while the dry-run compose wires BTC/ETH/SOL as full primary entries (300 USDT notional, `max_spread_bps=10`, empty `TRADING_SHADOW_SYMBOLS`) so rehearsals track the intended live posture.
...
- **Shared trading decision logic** – Trading now routes through `app/trading/decision.py::decide_bar`, applying spread/hold/SL/TP checks consistently across live loops and offline sweeps; `TRADING_MODELS` accepts guard fields (max spread, stop loss, take profit, max hold) and the default compose applies the same 300 USDT, 10 bps, primary policy config to BTC/ETH/SOL with `shadow_mode=false`. Decision payloads now carry price/spread fields so exits respect execution context.
...
- `app/trading/service.py` consumes that queue, keeps per-symbol state (`app/trading/state.py`) with file/Redis/Postgres backends, logs audit events (`app/trading/audit.py`), and routes orders through the CCXT adapter with spread guardrails and dry-run toggles. The shared `app/trading/decision.py::decide_bar` applies entry/exit/hold/SL/TP/spread checks consistently across live loops and offline sweeps; `TRADING_MODELS` entries accept guard fields and the compose defaults mirror the live ladder (BTC/ETH/SOL primary entries, 300 USDT notional, 10 bps spread cap, `shadow_mode=false`).
...
- **Trading controls**: `app/trading/service.py` now enforces kill-/safe-mode envs, runtime risk gating (turnover/exposure/cooldown/per-symbol limits), Redis intent ledger dedupe, reconciliation safe-mode latching, programmable deadlock policies, richer audit payloads (expected/net PnL, quote source, `loss_guard` hits), and HMAC-signed logging. Stage-0 overrides now reduce `cooldown_minutes_after_exit` to 1, `cooldown_minutes_after_loss` to 5, and add `TRADING_LAST_TS_GRACE_BARS` plus `TRADING_DECISION_MAX_AGE_SECONDS` so stale queue entries get skipped while the dry-run compose mirrors the live ladder with BTC/ETH/SOL all marked as primary symbols (`TRADING_SHADOW_SYMBOLS=[]`). Reports from `analysis.*` CLIs (including the new `analysis.live_readiness_check`, `analysis.acceptance_trade_proof`, `analysis.exit_attribution_report`, and `analysis.project_stance_snapshot`) plus Prometheus metrics (`trading_deadlock_*`, `trading_safe_mode_latched`, `trading_risk_blocked_total`, `trading_intent_ledger_state_total`, `trading_reconcile_runs_total`) mirror the deployment contract's live invariants.
