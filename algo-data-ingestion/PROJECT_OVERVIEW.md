...
- **Shared trading decision logic** – Trading now routes through `app/trading/decision.py::decide_bar`, applying spread/hold/SL/TP checks consistently across live loops and offline sweeps; `TRADING_MODELS` accepts guard fields (max spread, stop loss, take profit, max hold) and the default compose includes them for the ETH 1m policy. Decision payloads now carry price/spread fields so exits respect execution context.
...
- `app/trading/service.py` consumes that queue, keeps per-symbol state (`app/trading/state.py`) with file/Redis/Postgres backends, logs audit events (`app/trading/audit.py`), and routes orders through the CCXT adapter with spread guardrails and dry-run toggles. The shared `app/trading/decision.py::decide_bar` applies entry/exit/hold/SL/TP/spread checks consistently across live loops and offline sweeps; `TRADING_MODELS` entries accept guard fields and the compose defaults set them for the ETH 1m policy.
...
- **Trading controls**: `app/trading/service.py` now enforces kill-/safe-mode envs, runtime risk gating (turnover/exposure/cooldown/per-symbol limits), Redis intent ledger dedupe, reconciliation safe-mode latching, programmable deadlock policies, and HMAC-signed audit logging. Reports from `analysis.*` CLIs plus Prometheus metrics (`trading_deadlock_*`, `trading_safe_mode_latched`, `trading_risk_blocked_total`, `trading_intent_ledger_state_total`, `trading_reconcile_runs_total`) mirror the deployment contract's live invariants.
