...
> Update 2026-01-01: Refreshed the compose defaults (ETH profit-trailing + adjusted stop-loss) and stage-0 trigger overrides (BTC entries suppressed via `entry_threshold=1.0`).
> Update 2025-12-30: Stage-0 sizing/risk limits bumped to capital 200 (`equity_fraction=0.33`, `max_equity_fraction=0.35`) and now include vol-aware stop shaping (`min_stop_loss_pct`/`hard_stop_loss_pct`/`vol_stop_rvol_mult`) plus optional quote-based price monitoring (`TRADING_PRICE_MONITOR_INTERVAL_SECONDS`) to enforce exits even when decision payloads stall.
...
1. Before booting containers, confirm `TRADING_MODELS` matches the active stage bundle or compose defaults (BTC/ETH/SOL configured, tight spreads `max_spread_bps=8`, per-symbol stop-loss/profit-trailing/max-hold guards, and any churn-reduction knobs like `disable_prob_exits` and `entry_rsi_min`/`entry_macd_min`). Keep `TRADING_SHADOW_SYMBOLS=[]` so rehearsals exercise the real policy mix. Verify `configs/runtime_overrides/risk_limits_stage_0.yaml` shows `sizing_mode: equity_fraction` (`capital=200`, `equity_fraction=0.33`, `max_equity_fraction=0.35`, `compounding_step_usd=5`, `max_total_notional=80`) plus `cooldown_minutes_after_exit: 2` / `cooldown_minutes_after_loss: 5` and stop shaping (`min_stop_loss_pct`, `hard_stop_loss_pct`, `vol_stop_rvol_mult`).
1. Run a quick trigger preflight before booting containers so dry-runs do not start with zero coverage: `python scripts/trigger_preflight.py --contract configs/canonical_training_contract_market_multi_3symbol_1m.yaml --policy configs/final_trigger_policy.yaml --model-dir models/base_xgb_h120_calmon_spread0 --max-rows 5000` (add `--symbol ETH/USDT` to focus on one market).
1. Ensure the deployed model bundle exists under `models/base_xgb_h120_calmon_spread0` (Docker copies `models/` into `/opt/models` during build; `MODELS_ROOT=/opt/models` at runtime).
