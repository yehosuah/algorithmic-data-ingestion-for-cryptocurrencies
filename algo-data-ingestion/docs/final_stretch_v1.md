...
- **Status**: `final_equity 1.84`, Sharpe 28.7, 711 toggles at threshold 0.95 with RSS spike gate share ≈2 %. Oct 2025 replay now fires 5 870 deployable trades (`gate_coverage ≈ 16 %`) under the eased manifest (`prob ≥ 0.5`, `rvol_20 ≤ 5e-4`, `min_hold 10`).
- **Actions**
  ...
  3. Keep the blender manifest aligned with the latest base thresholds and confirm Oct 2025 replay maintains equity ≥1.2 with turnover within ±25 % of the current run before sign-off.
...
- Metrics to expose: equity curve, turnover, gate activation rate (`model_gate_coverage_ratio`), RSS coverage (`model_rss_minute_spike_share`), probability σ (`model_probability_sigma`), plus their manifest thresholds.
- Alert thresholds:
