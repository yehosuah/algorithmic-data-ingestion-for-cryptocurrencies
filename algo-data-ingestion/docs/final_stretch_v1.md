...
## Logistic Blender
1. **Dataset rebuild**: generate a year-wide blender matrix before applying final gates; include raw `base_prob`, `tcn_prob`, RSS features, and additional engineered spreads (e.g., `hl_spread_z`, `rvol_delta`).
...
5. **Social-signal audit**: require ≥2 % coverage for RSS/Reddit features on the training slice before they are included; otherwise drop them to keep the logistic fit stable.
