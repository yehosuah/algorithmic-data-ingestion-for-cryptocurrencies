...
- **Base XGB (H120, Calmon relaxed)** – `models/base_xgb_h120_calmon_spread0`
  ...
  - Oct 2025 replay registered zero deployable trades despite healthy training-gate equity.
- **TCN suite (Calmon relaxed)** – `models/tcn_h{60,120,180}_calmon_relaxed`
  ...
  - Matching Oct 2025 zero-coverage outcome requires retuning alongside the base model.
- **Blender (H120 elastic-net)** – `models/blender_h120_v6`
  ...
  - Forward replay using `datasets/blender_matrix_2025-10_to_2025-11_with_preds.parquet` produced zero deployable toggles; align gating with the retuned base/TCN thresholds.
...
- `models/oos_replay_oct_nov_2025.json` – Forward replay comparing relaxed vs deployable gates across base/TCN/blender.
...
1. **Forward Validation** – Replay Oct–Nov 2025 (and beyond) for base, TCN, blender using the deployable gate; retune thresholds/fallbacks so equity ≥1.2 and turnover deviations <±25 % from baseline instead of the current zero-coverage outcome.
