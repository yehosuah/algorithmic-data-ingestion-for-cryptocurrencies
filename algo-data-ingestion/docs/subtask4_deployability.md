...
> Update 2025-11-13: Folded in the sanitizer + symbol-gate workflow and the feature parity helpers (`export_feature_slice.py`, `compare_feature_stats.py`) so deployability checkpoints cite the same datasets/gates monitored by scheduler + trading.
...
- `datasets/market_multi_3symbol_1m.parquet` + `release/symbol_gates/market_multi_3symbol_1m.json` – Sanitized BTC/ETH/SOL feed (built via `training.data.sanitize_market_dataset`) and the symbol-aware gate payload generated by `scripts/compute_symbol_gate_config.py`, ensuring scheduler/trading share the same `hl_spread`, `rvol`, and liquidity caps as training.
...
6. **Feature Parity Proof** – For every threshold tweak, export a scheduler slice and stash the diff (`python scripts/compare_feature_stats.py --train datasets/market_multi_3symbol_1m.parquet --live /tmp/features_debug.parquet --out release/calibration/latest/deployability_parity.json`) so sign-off packets include concrete `hl_spread`, `hl_spread_z`, `rvol_20`, and `base_prob` drift.
