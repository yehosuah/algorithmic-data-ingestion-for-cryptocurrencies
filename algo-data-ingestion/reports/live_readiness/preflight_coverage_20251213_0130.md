# Preflight Coverage

Generated at: 2025-12-13T01:30:24.040636+00:00
Contract: /Users/yehosuahercules/Desktop/EcuacionesDiferenciales/CCXT_Testing/algo-data-ingestion/configs/deployment_portfolio_contract.yaml
Features: data/features_labels_regimes_market_multi_3symbol_1m.parquet

## ETH/USDT
- Model: xgb_primary
- Samples: 151
- Prob quantiles: p50=0.5657 p90=0.6844 p95=0.6844 p99=0.6844
- Prob gate min: 0.49 | fraction>=gate: 0.7351
- Entry/exit: None / None | fraction between: 0.0000
- Gate coverage ratio: 0.7351
- Implied trade proxy (signal changes): 12

## BTC/USDT
- Model: xgb_primary
- Samples: 180
- Prob quantiles: p50=0.5002 p90=0.6844 p95=0.6844 p99=0.6844
- Prob gate min: 0.49 | fraction>=gate: 0.6000
- Entry/exit: None / None | fraction between: 0.0000
- Gate coverage ratio: 0.6000
- Implied trade proxy (signal changes): 9

## SOL/USDT
- Model: xgb_primary
- Samples: 181
- Prob quantiles: p50=0.5002 p90=0.6844 p95=0.6844 p99=0.7031
- Prob gate min: 0.49 | fraction>=gate: 0.6133
- Entry/exit: None / None | fraction between: 0.0000
- Gate coverage ratio: 0.6133
- Implied trade proxy (signal changes): 17
